...
- 查詢頁可查單品、材料、上次成交價、近三天成交筆數，並支援單配方更新
- 排行頁使用預先計算的 `profits`，可切換 `全伺服器 / 鳳凰` 材料成本口徑
- 排行頁固定多顯示一欄 `單件成本(鳳凰)`
- 收藏品成本頁使用 `collectable_rewards`，也支援 `全伺服器 / 鳳凰` 成本口徑
- 全量更新支援進度面板、log 下載、中斷更新
- 成交筆數目前來自 Universalis `/history`，不是 velocity
