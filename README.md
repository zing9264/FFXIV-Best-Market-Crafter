...
- `查詢頁`
  - 輸入道具名稱或 ID
  - 顯示成品價格、素材最低價、素材鳳凰價、素材總和、單件成本、上次成交價、近三天成交筆數
  - 可直接更新該配方的成品與素材價格

- `價差排行`
  - 讀預先算好的 `profits`
  - 可依 `當前價差 / 當前獲利% / 過去獲利 / 過去獲利%` 排序
  - 可切換 `全伺服器 / 鳳凰` 材料成本口徑
  - 固定額外顯示 `單件成本(鳳凰)`

- `收藏品成本`
  - 讀 `collectable_rewards`
  - 顯示職業、等級、紫票、單件成本、每張紫票成本
  - 同樣支援 `全伺服器 / 鳳凰` 成本口徑切換
  - 固定額外顯示 `單件成本(鳳凰)`
...
目前不是只存一份資料，而是會預先算兩套材料成本口徑：

- `world='繁中服'`
- `world='鳳凰'`

所以切換排行頁物價口徑時，不是前端重算，而是直接切換查詢哪一批預先算好的 rows。
